...
function submitDatabaseWriteJob(query, parameters) {
    return new Promise(function(resolve, reject) {
        const databaseWriteJobId = uuidv1() + '-' + uuidv4();

        PENDING_DATABASE_WRITE_JOBS[databaseWriteJobId] = {resolve: resolve, reject: reject};
        ...
        if (cluster.isWorker) {
            ...
            .then(() => {
                finishPendingDatabaseWriteJob(databaseWriteJobId, null);
            })
            .catch((error) => {
...
*/
async function performDatabaseWriteJob(query, parameters) {
    try {
...
async function performDatabaseReadJob_ALL(query, parameters) {
    try {
...
async function performDatabaseReadJob_GET(query, parameters) {
    try {
...
function finishPendingDatabaseWriteJob(databaseWriteJobId, error) {
    if (PENDING_DATABASE_WRITE_JOBS.hasOwnProperty(databaseWriteJobId)) {
        ...
        if (error != null) {
            pendingDatabaseWriteJob.reject(error);
        } else {
            pendingDatabaseWriteJob.resolve();
        }
    }
}
